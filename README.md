...
For a media demo without an external download, set `media_url` to `http://mms-api:8000/demo/media/pixel.gif`. This bundled 1×1 GIF is reachable by Mbuni inside Docker; the same file can be viewed from your Mac at `http://localhost:8000/demo/media/pixel.gif`. Omitting `media_url` sends text-only MMS. `https://example.com/image.jpg` was an illustrative placeholder, not a supplied image.
