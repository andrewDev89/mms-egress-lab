...
DEFAULT_MAX_ATTEMPTS = int(os.getenv("DEFAULT_MAX_ATTEMPTS", "100"))
...
BLAST_CHUNK_SIZE = int(os.getenv("BLAST_CHUNK_SIZE", "300"))
...
WORKER_SEND_TPS = int(os.getenv("WORKER_SEND_TPS", "600"))
SEND_ATTEMPT_BACK_OFF_SECONDS = float(os.getenv("SEND_ATTEMPT_BACK_OFF_SECONDS", "2"))
if WORKER_SEND_TPS <= 0 or WORKER_BATCH_SIZE <= 0 or WORKER_CONCURRENCY <= 0:
    raise ValueError("Worker sending rate, batch size, and concurrency must be positive")
if SEND_ATTEMPT_BACK_OFF_SECONDS <= 0:
    raise ValueError("SEND_ATTEMPT_BACK_OFF_SECONDS must be positive")
